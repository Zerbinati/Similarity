...
#define USE_CUSTOM_HASHER
    ...
    #include "sparsehash/dense_hash_map"
        ...
        template<typename tKey, typename tVal> class HypnoSMap : public google::dense_hash_map<tKey, tVal, KeyHasher, KeyHasher>
        {
        public:
            HypnoSMap(tKey emptyKey, tKey deletedKey)
            {
    ...
    #else
        template<typename tKey, typename tVal> class HypnoSMap : public google::dense_hash_map<tKey, tVal>
        {
        public:
            HypnoSMap(tKey emptyKey, tKey deletedKey)
            {
    ...
    template<typename tVal> class HypnoSKeyMap : public HypnoSMap<Stockfish::Key, tVal>
    {
    public:
        HypnoSKeyMap() : HypnoSMap<Key, tVal>((Key)0, (Key)-1)
        {
        }
    };
#else
    #include <unordered_map>
    template<typename tKey, typename tVal> using HypnoSMap = unordered_map<tKey, tVal>;
    template<typename tVal> using HypnoSKeyMap = HypnoSMap<Key, tVal>;
#endif
...
namespace Experience
{
    ...
    // Typedefs
    ////////////////////////////////////////////////////////////////
    typedef HypnoSKeyMap<ExpEntryEx*> ExpMap;
    typedef HypnoSKeyMap<ExpEntryEx*>::iterator ExpIterator;
    typedef HypnoSKeyMap<ExpEntryEx*>::const_iterator ExpConstIterator;
